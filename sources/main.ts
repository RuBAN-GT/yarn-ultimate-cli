...
export async function main({
  ...
  async function exec(cli: Cli<CommandContext>): Promise<void> {
    ...
    const ignoreNode = miscUtils.parseOptionalBoolean(
    ...
    const tryRead = (p: PortablePath) =>
      xfs.readFilePromise(p).catch(() => {
        return Buffer.of()
      })
    ...
    const isSameBinary = async () =>
      yarnPath === selfPath ||
      Buffer.compare(
        ...(await Promise.all([tryRead(yarnPath), tryRead(selfPath)]))
      ) === 0
    ...
    if (!ignorePath && !ignoreCwd && (await isSameBinary())) {
      ...
      for (const [name, plugin] of configuration.plugins.entries()) {
        if (
          pluginCommands.has(name.match(/^@yarnpkg\/plugin-(.*)$/)?.[1] ?? ``)
        )
